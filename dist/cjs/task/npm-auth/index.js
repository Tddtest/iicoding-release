...
var logoutNpm = () => {
  return new Promise((resolve) => {
    (0, import_utils.runAsync)("npm logout").then(() => {
      import_utils.logger.succeed({ text: "退出成功\r\n" });
      resolve(true);
    }).catch((err) => {
      resolve(false);
      import_utils.logger.fail({ text: `退出 npm 账户出错，请手动重试, ${err.message}` });
...
var loginNpm = (next, params) => {
  ...
  const result = (0, import_utils.runShellExec)("npm login");
  let isRunning = true;
  import_utils.logger.start({ text: "正在请求 npm 登录，如果因为网络原因长时间无结果反馈,请手动退出，重新登录\r\n", spinner: "simpleDots" });
  result.stdout.on("data", (r) => {
    ...
    if (r.includes("https://www.npmjs.org/login?") || r.includes("https://www.npmjs.com/login?")) {
      ...
      import_utils.logger.succeed({ text: "请点击下面链接进入网页登录， 认证成功之后会自动进入下一步流程\r\n" });
      console.log(r);
    } else {
      import_utils.logger.succeed(`npm 登录成功 ${r} 即将进入发布环节。`);
      next(params);
    }
  });
...
var handleNotLoggedIn = (error, next, params) => {
  if (error.message.includes("npm ERR! need auth This command requires you to be logged in.")) {
    import_utils.logger.fail({ text: "npm 未登录， 请选择是否登录" });
...
var handleToggleNpmUser = async (next, params) => {
  import_utils.logger.start({ text: "正在退出当前npm账号" });
...
var npmAuth = async (next, params) => {
  (0, import_global_register.registerSigint)();
  import_utils.logger.start({ text: "正在验证是否登录 npm" });
  try {
    const res = await (0, import_utils.runAsync)("npm who");
    import_utils.logger.succeed(`当前npm登录账号为：${res.stdout}\r
`);
